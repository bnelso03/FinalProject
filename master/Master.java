...
public class Master implements Runnable {
  ...
  // Thread pooling? Just sort of picks from this list, if there's a running
  // server on that port we're good.
  private static List<Integer> utilityPorts = Arrays.asList(32006, 32007, 32008, 32009, 32010, 32011, 32012, 32013,
      32014, 32015);
    ...
    for (int port : activeUtilityPorts) {
      try {
        ...
        if (response.equals("AVAILABLE")) {
          ...
          System.out.println("[MASTER] Utility at port " + port + " is AVAILABLE");
          ; // DO NOT REMOVE. It breaks everything, I do not know why it breaks everything,
            // but LEAVE IT!!!!
        } else {
          System.out.println("[MASTER] Utility at port " + port + " is not responding");
        }
